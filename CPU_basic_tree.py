...
def simulate(grid, policy, symbols = CPUSYMBOL):
...
def get_opponent(grid): # Get the symbol for the opponent. Returns 'x' if none found.
    symbols = set(chain.from_iterable(grid))
    if API.EMPTYSYMBOL in symbols:
        symbols.remove(API.EMPTYSYMBOL)
    if API.CPUSYMBOL in symbols:
        symbols.remove(API.CPUSYMBOL)
    if len(symbols) == 1:
...
def tree(grid):
    ...
    score_dict = {}
    opponent_symbol = get_opponent(grid)
    ...
    for choice in available:
        ...
        choice_grid = API.insert_symbol(CPUSYMBOL,choice_grid,*choice)

        win = API.checkWin(choice_grid)

        if win == CPUSYMBOL:  # Direct win --> best choice
            return choice
